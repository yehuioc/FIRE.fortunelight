...
```bash
.venv/bin/python backend/seed_demo.py --confirm-reset
```

默认 60 天 · 平均日花销 ≈ ¥70 · 平均日收入 ≈ ¥500。
空账本可省略 `--confirm-reset`；已有交易时命令会默认拒绝，只有显式确认后才会先备份到 `data/backups/`，再替换为演示数据。
...
run.sh               一键启动脚本
run.ps1              Windows PowerShell 一键启动
```
...
所有数据和自动备份均存储在本地 `data/`，**不上传任何服务器**。后端仅监听 `127.0.0.1`。

...
(() => {
  ...
  class LifeGrid {
    constructor(canvas, audio = null) {
      ...
      this.assetLit = 0;        // 起始资产带来的自由格数（在 income 之前）
    ...
    cameraConverged() {
      return Math.abs(this.camera.scale - this.cameraTarget.scale * this.userZoom) < 0.003
          && Math.abs(this.camera.tx    - this.cameraTarget.tx    * this.userZoom) < 0.4
          && Math.abs(this.camera.ty    - this.cameraTarget.ty    * this.userZoom) < 0.4;
    }
    ...
    loop() {
      const tick = (now) => {
        ...
        const targetScale = this.cameraTarget.scale * this.userZoom;
        this.camera.scale = lerp(this.camera.scale, targetScale, k);
        this.camera.tx    = lerp(this.camera.tx,    this.cameraTarget.tx * this.userZoom,    k);
        this.camera.ty    = lerp(this.camera.ty,    this.cameraTarget.ty * this.userZoom,    k);
    ...
    /** 鼠标聚焦缩放 · mx/my = 相对 canvas 的 CSS 像素 · dir = 1 放大 / -1 缩小 */
    zoomAt(mx, my, dir) {
      const ZOOM_MIN = 0.4, ZOOM_MAX = 6.0, ZOOM_STEP = 0.12;
      const oldZ = this.userZoom;
      const newZ = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, oldZ + dir * ZOOM_STEP));
      if (newZ === oldZ) return;
      const cx = this.cssW / 2;
      const cy = this.cssH / 2;
      const oldGridX = (mx - cx + cx * oldZ) / oldZ;
      const oldGridY = (my - cy + cy * oldZ) / oldZ;
      const offsetX = mx - cx + cx * newZ - newZ * oldGridX;
      const offsetY = my - cy + cy * newZ - newZ * oldGridY;
      this.userZoom = newZ;
      this.cameraTarget.tx = offsetX / newZ;
      this.cameraTarget.ty = offsetY / newZ;
      this.needsRedraw = true;
    }

...
(() => {
  ...
  const els = {
    ...
    canvas: $('#grid'),
    canvasWrap: $('.stage__canvas-wrap'),
    starfield: $('#starfield'),
    btnStageCollapse: $('#btn-stage-collapse'),
    ledger: $('.ledger'),
    stageZoomHint: $('#stage-zoom-hint'),
  };
  ...
  // 窄屏折叠 + 鼠标聚焦缩放
  // ============================================
  const stackedMQ = window.matchMedia('(max-width: 1080px)');
  let isCollapsed = false;

  stackedMQ.addEventListener('change', e => {
    if (!e.matches) {
      els.ledger.classList.remove('is-collapsed');
      els.btnStageCollapse.querySelector('svg').style.transform = '';
      isCollapsed = false;
      grid.userZoom = 1.0;
    }
  });

  els.btnStageCollapse.addEventListener('click', () => {
    isCollapsed = !isCollapsed;
    els.ledger.classList.toggle('is-collapsed', isCollapsed);
    const arrow = els.btnStageCollapse.querySelector('svg');
  ...
  els.canvasWrap.addEventListener('wheel', (e) => {
    ...
    grid.zoomAt(mx, my, -Math.sign(e.deltaY));
    els.stageZoomHint.classList.add('is-visible');
    clearTimeout(grid._zoomHintTimer);
    grid._zoomHintTimer = setTimeout(() => els.stageZoomHint.classList.remove('is-visible'), 1800);
  }, { passive: false });
  ...
  els.canvas.addEventListener('dblclick', () => {
    if (!stackedMQ.matches) return;
    grid.userZoom = 1.0;
    grid.cameraTarget.tx = 0; grid.cameraTarget.ty = 0;
